...
const defaultExam = {
  id: 'default-exam-1',
  name: 'Đề Thi Vào 10 - Tiêu Chuẩn (50 Câu)',
  ...
  return (
    <button onClick={() => onSelect(idx)} disabled={isRevealed || isSubmitted} className={btnCls}>
      <div className="flex items-center gap-3 relative z-10 w-full pr-16">
        <span className={`w-8 h-8 shrink-0 rounded-lg flex items-center justify-center font-black text-sm transition-all duration-200 ${
...
const ExamPlayer = ({ exam, onExit, onComplete }) => {
  ...
  const showTrendyCompliment = useCallback(() => {
    const praises =["10 ĐIỂM KHÔNG CÓ NHƯNG!", "HẾT NƯỚC CHẤM!", "NHỨC CÁI NÁCH!", "OUT TRÌNH LUÔN!", "ĐỈNH CỦA CHÓP!"];
  ...
  const handleManualSubmit = useCallback(() => {
    ...
    if (window.confirm("Bạn có chắc chắn muốn nộp bài sớm không? Các câu chưa làm sẽ được tính là sai.")) {
      processSubmit();
    }
    ...
    <>
        ...
        <div className="space-y-4 flex-1 mt-4">
          <div className="relative group">
            <input 
              ...
              className={`w-full bg-black/40 border-2 rounded-2xl p-5 pl-12 font-bold transition-all duration-300 outline-none focus:ring-4 focus:ring-cyan-500/10 ${
      ...
      {currentQ.type === 'word_order' && (
        <div className="space-y-6 flex-1 mt-2">
          <div className={`min-h-[80px] w-full bg-black/40 border-2 rounded-2xl p-5 transition-all duration-300 flex flex-wrap gap-2 items-start content-start ${
            ...
            {(userAns ||[]).map((wordIdx, pos) => (
              <button key={`sel-${wordIdx}-${pos}`} onClick={() => handleWordOrderClick(currentQ.id, wordIdx, true)} disabled={isRevealed || isSubmitted}
                className={`px-4 py-2 rounded-lg font-bold transition-all duration-200 shadow-sm active:scale-95 ${
